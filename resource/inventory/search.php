...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <?php 
	   ...
	   $searchType=$_GET['select'];
	   if($searchType=="项目名称")
	   {
		    $result = mysql_query("select ProjectName,StartTime,EndTime,Leader,ProjectId from project where ProjectName='$searchKey'");  //读取数据 
	   }else if($searchType="负责人")
	   {
		    $result = mysql_query("select ProjectName,StartTime,EndTime,Leader,ProjectId from project where Leader='$searchKey'");  //读取数据 
	   }else{
		    $result = mysql_query("select ProjectName,StartTime,EndTime,Leader,ProjectId from project where Industry='$searchKey'");  //读取数据 
	   }
