...
<table class="table table-bordered table-hover definewidth m10">
	...
	<?php 
	   ...
	   if($searchType=="工号")
	   {
		    $result = mysql_query("select WorkId,Name,Duration,Section from worker where WorkId='$searchKey'");  //读取数据 
	   }else{
		    $result = mysql_query("select WorkId,Name,Duration,Section from worker where Name='$searchKey'");  //读取数据 
	   }
      //检测用户名及密码是否正确
